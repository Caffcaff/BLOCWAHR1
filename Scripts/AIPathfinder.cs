 ...
 */
	 ...
	 */
		...
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != temp) {
				SearchPath();
				return repathRate;
			} else {
		...
		Vector3 temp = new Vector3 (0, 0, 0);
		if (target == temp) throw new System.InvalidOperationException("Target is null");
